...
export default function GamePage() {
  ...
  const [championshipOver, setChampionshipOver] = useState<{
    champion: string;
  } | null>(null);
  const [playerLeftMsg, setPlayerLeftMsg] = useState<string | null>(null);
  const [specialCardMsg, setSpecialCardMsg] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (socket) {
      ...
      socket.on("card_played", ({ playerId, card }) => {
        ...
        if (card.number === 1) setSpecialCardMsg("HOLD ON!");
        else if (card.number === 2) setSpecialCardMsg("PICK TWO!");
        else if (card.number === 5) setSpecialCardMsg("PICK THREE!");
        else if (card.number === 8) setSpecialCardMsg("SUSPENSION!");
        else if (card.number === 14) setSpecialCardMsg("GENERAL MARKET!");
        else if (card.number === 20) setSpecialCardMsg("WHOT!");

        if ([1, 2, 5, 8, 14, 20].includes(card.number)) {
          setTimeout(() => setSpecialCardMsg(null), 2000);
        }
      ...
      socket.on(
        "game_over",
        (data: { winner: string; message?: string; room: Room }) => {
      ...
      socket.on(
        "round_ended",
        (data: { roundResult: RoundResult; room: Room }) => {
          if (data.room.id === roomId) {
            setRoom(data.room);
      ...
      socket.on(
        "championship_over",
        (data: { champion: string; room: Room }) => {
          if (data.room.id === roomId) {
            setRoom(data.room);
          ...
          <div className="space-y-3 mb-8">
            ...
            {Array.from({ length: room.maxPlayers - room.players.length }).map(
              (_, i) => (
                <div
                  key={i}
                  className="flex items-center justify-center p-4 bg-black/20 rounded-2xl border border-dashed border-white/10"
                >
      ...
      <main className="min-h-screen bg-emerald-900 flex items-center justify-center p-4">
        ...
        >
          ...
          <p className="text-gray-400 mb-8">
            {isHost
              ? "The room is full! Choose how you want to play."
          ...
          {isHost ? (
            <div className="grid grid-cols-1 gap-4">
              ...
              >
                <span className="block text-xl font-black text-white group-hover:text-white mb-1">
                  Challenge
                </span>
                <span className="block text-sm text-gray-400 group-hover:text-emerald-100">
                  Classic Whot. First to finish wins.
                </span>
              </button>
              <button
              ...
              >
                ...
                <span className="block text-sm text-gray-400 group-hover:text-emerald-100">
                  Elimination rounds until one champion remains.
                </span>
              </button>
            </div>
          ) : (
    ...
    <main className="min-h-screen bg-emerald-900 overflow-hidden relative font-sans">
      ...
      <AnimatePresence>
        {specialCardMsg && (
          ...
          >
            <div className="bg-amber-500 text-white px-12 py-6 rounded-full shadow-[0_0_50px_rgba(245,158,11,0.5)] border-4 border-white animate-pulse">
              <span className="text-4xl sm:text-6xl font-black uppercase tracking-tighter italic">
                {specialCardMsg}
              </span>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            initial={
              lastAction.type === "play"
                ? {
                    x:
                      lastAction.playerId === socket?.id
                        ? 0
                        : lastAction.playerId === room?.players[0]?.id
                          ? -300
                          : 300,
              ...
              {lastAction.type === "play" && lastAction.card ? (
                <div className="flex flex-col items-center">
                  <span className="text-2xl font-black text-amber-900">
                    {lastAction.card.number}
                  </span>
                  <ShapeIcon
                    shape={lastAction.card.shape}
      ...
      <div className="fixed top-0 left-0 right-0 z-40 p-4 flex items-center justify-between pointer-events-none">
        <div className="bg-black/60 backdrop-blur-md border border-white/10 px-4 py-2 rounded-xl flex items-center gap-3 pointer-events-auto shadow-xl">
          <div className="w-8 h-8 bg-emerald-600 rounded-lg flex items-center justify-center shadow-lg">
            <Users className="w-4 h-4 text-white" />
          </div>
          <div className="hidden sm:block">
            <span className="block text-[10px] font-black uppercase tracking-widest text-emerald-400 leading-none mb-1">
              Room
            </span>
        ...
        <button
          onClick={handleBackToLobby}
          className="bg-black/60 backdrop-blur-md border border-white/10 px-4 py-2 rounded-xl text-white font-bold text-sm flex items-center gap-2 hover:bg-white/10 transition-all pointer-events-auto shadow-xl"
        >
      ...
      {/* Game Layout */}
      <div className="relative w-full min-h-screen flex flex-col items-center justify-between p-4 pt-20 sm:p-8 sm:pt-24 overflow-y-auto">
        {/* Spectator Message */}
        ...
        <div className="flex flex-col items-center gap-2">
          {otherPlayers.find(
            (p) => p.position === (room.players.length === 2 ? 1 : 2),
          ) && (
            <PlayerSeat
              player={
                otherPlayers.find(
              ...
              isEliminated={room.eliminatedPlayerIds.includes(
                otherPlayers.find(
                  (p) => p.position === (room.players.length === 2 ? 1 : 2),
                )?.id || "",
              )}
        ...
        <div className="flex-1 w-full flex items-center justify-between max-w-6xl">
          ...
          <div className="w-32 h-64 flex items-center justify-center">
            {otherPlayers.find((p) => p.position === 1) &&
              room.players.length > 2 && (
                <PlayerSeat
                  ...
                  isTurn={
                    ...
                    room.players.findIndex(
                      (p) =>
                        p.id === otherPlayers.find((p) => p.position === 1)?.id,
                    )
                  }
                  isEliminated={room.eliminatedPlayerIds.includes(
                    otherPlayers.find((p) => p.position === 1)?.id || "",
                  )}
                  vertical
                />
              )}
          ...
          <div className="flex flex-col sm:flex-row items-center gap-8 sm:gap-16">
            ...
            >
              ...
              <Card
                faceDown
                className="shadow-2xl transform -rotate-3 translate-x-1"
              />
              <Card
                faceDown
                className="absolute top-0 left-0 shadow-2xl transform rotate-2 -translate-x-1"
              />
              ...
              {isMyTurn &&
                room.gameState.generalMarketTurns &&
                room.gameState.generalMarketTurns > 0 && (
            ...
            <div className="relative">
              ...
              {room.gameState.requestedShape && (
                ...
                >
                  <ShapeIcon
                    shape={room.gameState.requestedShape}
                    className="w-6 h-6"
                  />
                </motion.div>
              )}
          ...
          {/* Right Player */}
          <div className="w-32 h-64 flex items-center justify-center">
            {otherPlayers.find((p) => p.position === 3) &&
              room.players.length === 4 && (
                <PlayerSeat
                  ...
                  isTurn={
                    ...
                    room.players.findIndex(
                      (p) =>
                        p.id === otherPlayers.find((p) => p.position === 3)?.id,
                    )
                  }
                  isEliminated={room.eliminatedPlayerIds.includes(
                    otherPlayers.find((p) => p.position === 3)?.id || "",
                  )}
                  vertical
                />
              )}
        ...
        <div className="w-full max-w-4xl flex flex-col items-center gap-6 pb-4">
          <div className="flex items-center gap-4">
            ...
            >
              <div
                className={`w-3 h-3 rounded-full ${isMyTurn ? "bg-white animate-pulse" : "bg-gray-600"}`}
              />
              <span className="text-sm font-black uppercase tracking-widest">
                {room.gameState.generalMarketTurns &&
                room.gameState.generalMarketTurns > 0
                  ...
                  : isMyTurn
                    ? "Your Turn"
                    : `${room.players[room.gameState.currentPlayerIndex]?.username || "Someone"}'s Turn`}
              </span>
            </div>
          </div>
          ...
          <div className="relative w-full group">
            ...
            {myPlayer && myPlayer.cards.length >= 9 && (
              <>
                <button
                  onClick={() => scrollCards("left")}
                  className="hidden sm:flex absolute left-0 top-1/2 -translate-y-1/2 z-30 w-10 h-10 bg-black/60 backdrop-blur-md border border-white/10 rounded-full items-center justify-center text-white hover:bg-emerald-600 transition-all shadow-xl -translate-x-4 opacity-0 group-hover:opacity-100"
                >
                  <ChevronLeft className="w-6 h-6" />
                </button>
                <button
                  onClick={() => scrollCards("right")}
                  className="hidden sm:flex absolute right-0 top-1/2 -translate-y-1/2 z-30 w-10 h-10 bg-black/60 backdrop-blur-md border border-white/10 rounded-full items-center justify-center text-white hover:bg-emerald-600 transition-all shadow-xl translate-x-4 opacity-0 group-hover:opacity-100"
                >
            ...
            <div
              ...
              className="relative w-full overflow-x-auto pt-8 pb-4 scrollbar-hide scroll-smooth"
            >
      ...
      <AnimatePresence>
        {showShapeSelector && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            ...
            >
              <h2 className="text-2xl font-black text-amber-900 mb-2 tracking-tighter uppercase">
                Pick a Shape
              </h2>
              <p className="text-amber-800/60 text-sm font-medium mb-8">
                You played a WHOT card! Choose the next shape to continue.
              </p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl">
            ...
            >
              ...
              <p className="text-emerald-400 text-xl font-bold mb-8">
                {gameOver.winner
                  ? gameOver.winner === myPlayer?.username
                    ? "YOU WON!"
                    : `${gameOver.winner} Won!`
                  : "Game Ended"}
              </p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl">
            ...
            >
              <div className="text-center mb-8">
                <h2 className="text-3xl font-black text-white mb-1 tracking-tighter uppercase">
                  Round {room.currentRound} Ended
                </h2>
                <p className="text-emerald-500 font-bold uppercase tracking-widest text-xs">
                  {roundResult.winner} Won the round!
                </p>
              </div>
              ...
              <div className="space-y-3 mb-8">
                {roundResult.scores.map((s) => (
                  <div
                    key={s.username}
                    className={`flex items-center justify-between p-4 rounded-2xl border ${s.username === roundResult.eliminated?.username ? "bg-red-500/10 border-red-500/30" : "bg-white/5 border-white/10"}`}
                  >
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {s.username === roundResult.eliminated?.username && (
                        <span className="bg-red-600 text-white text-[8px] font-black uppercase px-2 py-1 rounded tracking-widest">
                          Eliminated
                        </span>
                      )}
                      {s.username === roundResult.winner && (
                        <span className="bg-emerald-600 text-white text-[8px] font-black uppercase px-2 py-1 rounded tracking-widest">
                          Qualified
                        </span>
                      )}
                    </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95 backdrop-blur-2xl">
            ...
            >
              ...
              <p className="text-amber-500 text-lg font-black mb-6 uppercase tracking-widest">
                {championshipOver.champion === myPlayer?.username
                  ? "YOU ARE THE CHAMPION!"
                  : `${championshipOver.champion} is the Champion!`}
              </p>
              ...
              <div className="space-y-2 mb-8 max-h-[40vh] overflow-y-auto pr-2 scrollbar-hide">
                {room.playerRanking.map((r, idx) => (
                  <div
                    key={r.username}
                    className={`flex items-center justify-between p-3 rounded-xl border ${idx === 0 ? "bg-amber-500/10 border-amber-500/30" : "bg-white/5 border-white/10"}`}
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`text-xl font-black ${idx === 0 ? "text-amber-500" : "text-gray-500"}`}
                      >
                        #{r.rank}
                      </span>
...
function PlayerSeat({
  ...
  return (
    <div
      className={`flex ${vertical ? "flex-col" : "flex-row"} items-center gap-4 ${isEliminated ? "opacity-40 grayscale" : ""}`}
    >
      <div
        className={`
      ...
      <div
        className={`flex flex-col ${vertical ? "items-center text-center" : "items-start text-left"}`}
      >
        <div className="flex items-center gap-2 mb-1">
          ...
          {isEliminated && (
            <span className="text-[8px] font-black bg-red-500/20 text-red-400 border border-red-500/30 px-1 rounded uppercase tracking-tighter">
              Eliminated
            </span>
          )}
          {player.isBot && !isEliminated && (
            <span className="text-[8px] font-black bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-1 rounded uppercase tracking-tighter">
              Bot
            </span>
          )}
        </div>
        {!isEliminated && (
          <div className="flex gap-1">
            {Array.from({ length: player.cards.length }).map((_, i) => (
              <div
                key={i}
                className="w-3 h-4 bg-red-800 rounded-sm border border-white/20 shadow-sm"
              />
            ))}
...
function ShapeIcon({ shape, className }: { shape: Shape; className?: string }) {
  ...
  switch (shape) {
    ...
    case "cross":
      return <Plus className={className} strokeWidth={4} fill="currentColor" />;
    case "whot":
      return <div className={`font-black ${className}`}>W</div>;
    default:
      return null;
  }
}
